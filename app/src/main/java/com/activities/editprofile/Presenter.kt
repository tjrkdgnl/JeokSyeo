...
class Presenter : EditProfileContract.EditProfilePresenter {
    override lateinit var view: EditProfileContract.EditProfileView
                    ...
                    if (loginCheck) {
                        ...
                        if (GlobalApplication.userInfo.nickName != view.getBindingObj().insertInfoEditText.text.toString()) {
                            check = Pattern.matches(
                                "^\\w+|[가-힣]+$", view.getBindingObj().insertInfoEditText.text.toString()
                            )
    ...
    private fun checkNickname(context: Context, right: Boolean) {
        if (right) {
            checkDuplicate = false
            view.checkOkButton()
            view.getBindingObj().checkNickNameText.visibility = View.VISIBLE
            view.getBindingObj().checkNickNameText.text =
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                view.getBindingObj().checkNickNameText.setTextColor(
            ...
            view.checkOkButton()
            view.getBindingObj().checkNickNameText.visibility = View.VISIBLE
            view.getBindingObj().checkNickNameText.text = context.getString(R.string.dontUseNickName)
