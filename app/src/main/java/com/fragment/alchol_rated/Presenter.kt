...
class Presenter :FragmentRated_Contract.BasePresenter {
    ...
    override lateinit var smoothScrollListener: Fragment_alcholRated.SmoothScrollListener
                    ...
                    it.data?.reviewList?.let { lst->
                        if(lst.isEmpty()){
                            lst.toMutableList().let { mlst->
                                mlst.add(ReviewList())
                                alcholRatedAdapter = AlcholRatedAdapter(context,mlst,smoothScrollListener)
                                view.getBinding().ratedRecyclerView.adapter =alcholRatedAdapter
                            }
                        }
                        else {
                            alcholRatedAdapter = AlcholRatedAdapter(context,lst.toMutableList(),smoothScrollListener)
                            view.getBinding().ratedRecyclerView.adapter =alcholRatedAdapter
                            initScrollListener()
                        }
                    }
    ...
    private fun pagination(){
        compositeDisposable.add(ApiGenerator.retrofit.create(ApiService::class.java)
            ...
            .subscribe({
                ...
                it.data?.reviewList?.let { lst->
                    if(lst.isNotEmpty()){
                        alcholRatedAdapter.updateItem(lst.toMutableList())
                    }
                }
