...
class Fragment_nickName : Fragment(), TextWatcher, View.OnKeyListener, View.OnClickListener {
    ...
    private fun rightNickName(name:String){
        val check = Pattern.matches("^\\w+|[가-핳]+$",name)
        ...
        if(!name.isEmpty()){
            if(check){
                ...
                binding.insertNameLinearLayout.background = resources.getDrawable(R.drawable.bottom_line_red,null)
                binding.checkNickNameText.setTextColor(resources.getColor(R.color.red,null))
            }
        }
        else{
            binding.insertNameLinearLayout.background = resources.getDrawable(R.drawable.bottom_line,null)
